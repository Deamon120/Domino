...
def computer_choices():
    while True:
        number = random.randint(-len(computer_pieces), len(computer_pieces))
        if number < 0:
            computer_choice = computer_pieces[abs(number)-1]
            if start_piece[0][0] == computer_pieces[abs(number)-1][1]:
                start_piece.insert(0, computer_choice)
                computer_pieces.remove(computer_choice)
                break
            elif start_piece[0][0] != computer_pieces[abs(number)-1][1] and start_piece[0][0] == computer_pieces[abs(number)-1][0]:
                start_piece.insert(0, computer_choice[::-1])
                computer_pieces.remove(computer_choice)
                break
            else:
                continue
            ...
            computer_choice = computer_pieces[abs(number) - 1]
            if start_piece[-1][1] == computer_pieces[abs(number)-1][0]:
                start_piece.append(computer_choice)
                computer_pieces.remove(computer_choice)
                break
            elif start_piece[-1][1] != computer_pieces[abs(number)-1][0] and start_piece[-1][1] == computer_pieces[abs(number)-1][1]:
                start_piece.append(computer_choice[::-1])
                computer_pieces.remove(computer_choice)
                break
            else:
                continue
